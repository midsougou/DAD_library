...
class MarkovianBenchMark: 
    ...
    def generate_dataset(self, n_states, n_symbols):
        ...
        good_data = train_generator.generate_all_sequence(n_good)

        bad_data = []
        for _ in range(self.n_anomaly_generator): 
            anomaly = MarkovSequenceGenerator(n_states=n_states, n_symbols=n_symbols, sequence_length=self.sequence_length).generate_all_sequences()
            bad_data.extend(anomaly)
        
        test_set = good_data + bad_data
        labels = [0 for _ in range(n_good)] + [1 for _ in range(n_bad)]
    ...
    def plot_distribution(self, n_symbols, n_states, labels, anomaly_scores, ax):      
        ...
        ax.set_title(f"{n_symbols} : {n_states}")
    ...
    def benchmark(self): 
        ...
        for i, (n_symbols, n_states) in enumerate(zip(self.n_symbols_list, self.n_states_list)): 
            ...
            anomaly_scores = self.model.scores(dataset.test)
            ...
            ax = fig.add_subplot(gs[i % n_plot, i > n_plot])
